...
/// FFI wrapper functions for Response
class ResponseAPI {
  late final ffi.DynamicLibrary _lib;

  ResponseAPI(this._lib);

  /// Creates a new Response FFI struct on the heap
  ffi.Pointer<ResponseFFI> create() {
    final createFn = _lib.lookupFunction<
        ffi.Pointer<ResponseFFI> Function(),
        ffi.Pointer<ResponseFFI> Function()>('response_create');
    return createFn();
  }

  /// Frees a Response FFI struct
  void free(ffi.Pointer<ResponseFFI> ptr) {
    final freeFn = _lib.lookupFunction<
        ffi.Void Function(ffi.Pointer<ResponseFFI>),
        void Function(ffi.Pointer<ResponseFFI>)>('response_free');
    freeFn(ptr);
  }

  /// Clones a Response FFI struct
  ffi.Pointer<ResponseFFI> clone(ffi.Pointer<ResponseFFI> ptr) {
    final cloneFn = _lib.lookupFunction<
        ffi.Pointer<ResponseFFI> Function(ffi.Pointer<ResponseFFI>),
        ffi.Pointer<ResponseFFI> Function(ffi.Pointer<ResponseFFI>)>('response_clone');
    return cloneFn(ptr);
  }

  /// Sends a Response proto model to Rust (returns FFI pointer)
  ffi.Pointer<ResponseFFI> send(Response msg) {
    final ptr = create();
    // Convert proto to FFI
    final ffiMsg = msg.toFFI();
    ptr.ref = ffiMsg.ref;
  ...
  /// Receives a Response proto model from Rust (does not free pointer)
  Response receive(ffi.Pointer<ResponseFFI> ptr) {
    return ResponseFromFFI.fromFFI(ptr);
  }

  /// Receives and frees a Response proto model from Rust
  Response receiveAndFree(ffi.Pointer<ResponseFFI> ptr) {
    final msg = receive(ptr);
    free(ptr);
    return msg;
  }
